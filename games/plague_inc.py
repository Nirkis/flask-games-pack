# -*- coding: utf-8 -*-
"""Plague Inc (realtime) — 1-4 игрока.

Живой пул ОБЩИЙ: сумма заражённых всех болезней ≤ (WORLD_POP − убитые).
Тип болезни игрок выбирает в лобби (внутри игры).
"""
import random
import threading
import time
...
GAME_META = {
    ...
    'modes': [1, 2, 3, 4],
    # extra_options убран: тип выбирается в лобби
    'order': 50,
}
...
DISEASE_TYPES = {
    'bacteria': {
        'name': '🦠 Бактерия',
        'desc': 'Быстрая и живучая. +30 % к стартовому распространению.',
        'spread': 1.30, 'kill': 0.80, 'vis': 1.10, 'vaccine': 1.20,
        'cost_mult': 1.00,
    ...
    'virus': {
        'name': '🧬 Вирус',
        'desc': 'Баланс. Сам мутирует — дёшево, но рискованно.',
        'spread': 1.10, 'kill': 1.00, 'vis': 1.30, 'vaccine': 1.00,
        'cost_mult': 1.00,
        'organ_mult': {'respiratory': 1.2, 'digestive': 0.9, 'nervous': 0.9,
                       'circulatory': 1.1, 'reproductive': 1.4, 'immune': 1.2},
        'passive': 'Каждые 15 тактов один открытый узел ±1 уровень (50/50).',
    },
    'parasite': {
        'name': '🪱 Паразит',
        'desc': 'Смертельный, но медленный. Тихий, пока его не замечают.',
        'spread': 0.80, 'kill': 1.50, 'vis': 0.80, 'vaccine': 0.90,
        'cost_mult': 1.10,
    ...
    'prion': {
        ...
        'spread': 0.60, 'kill': 2.00, 'vis': 0.50, 'vaccine': 0.70,
        'cost_mult': 1.20,
        'organ_mult': {'respiratory': 0.7, 'digestive': 0.7, 'nervous': 1.5,
                       'circulatory': 1.0, 'reproductive': 0.6, 'immune': 0.8},
        'passive': 'Вакцина против вас идёт ×0.7. После 30 такта смертность ×2.',
    },
}
...
SKILL_NODES = {
    ...
    'intestine': {'name': 'Кишечник', 'emoji': '🧶', 'x': 28, 'y': 28, 'max': 5,
                  ...
                  'effect': {'spread': 0.04}, 'desc': '+4 % распространение.',
                  'cost_tier': 'organ'},
    ...
    'diarrhea': {'name': 'Диарея', 'emoji': '💩', 'x': 20, 'y': 52, 'max': 1,
                 ...
                 'effect': {'spread': 0.30}, 'desc': 'КОМБО: +30 % распространение.',
                 'cost_tier': 'combo'},
    'vomit': {'name': 'Рвота', 'emoji': '🤢', 'x': 28, 'y': 52, 'max': 1,
              'requires': [('stomach', 3), ('intestine', 3)], 'system': 'digestive',
              'effect': {'spread': 0.15, 'kill': 0.15}, 'desc': 'КОМБО: +15 %/+15 %.',
              'cost_tier': 'combo'},
    'cholera': {'name': 'Холера', 'emoji': '💧', 'x': 24, 'y': 64, 'max': 1,
                'requires': [('diarrhea', 1), ('vomit', 1), ('intestine', 5)],
                'system': 'digestive',
                'effect': {'spread': 0.40, 'kill': 0.20}, 'desc': 'СУПЕР: +40 %/+20 %.',
                'cost_tier': 'combo'},
    ...
    'encephalitis': {'name': 'Энцефалит', 'emoji': '🧠', 'x': 40, 'y': 64, 'max': 1,
                     ...
                     'effect': {'kill': 0.40}, 'desc': 'СУПЕР: +40 % смертность.',
                     'cost_tier': 'combo'},
    'madness': {'name': 'Безумие', 'emoji': '☠', 'x': 40, 'y': 76, 'max': 1,
                ...
                'desc': 'ФИНАЛ: +50 %/+30 %, +30 % заметность.',
                'cost_tier': 'final'},
    ...
    'sepsis': {'name': 'Сепсис', 'emoji': '🔥', 'x': 56, 'y': 64, 'max': 1,
               ...
               'desc': 'ФИНАЛ: +55 % смертность, +20 % заметность.',
               'cost_tier': 'final'},
    'hemorrhage': {'name': 'Геморрагия', 'emoji': '🩸', 'x': 56, 'y': 76, 'max': 1,
                   ...
                   'desc': 'ЛЕГЕНДА: +35 %/+65 %, +35 % заметность.',
                   'cost_tier': 'legendary'},
    ...
    'gonads': {'name': 'Гонады', 'emoji': '🥚', 'x': 76, 'y': 28, 'max': 5,
               ...
               'effect': {'spread': 0.03, 'stealth': 0.03}, 'desc': '+3 %/+3 % скрытность.',
               'cost_tier': 'organ'},
    'hidden_transmission': {'name': 'Скрытая передача', 'emoji': '🩸', 'x': 68, 'y': 52, 'max': 1,
                            ...
                            'desc': 'КОМБО: +30 % распр., +20 % скрытность.',
                            'cost_tier': 'combo'},
    'vertical_transmission': {'name': 'Вертикальная передача', 'emoji': '🧬', 'x': 76, 'y': 52, 'max': 1,
                              ...
                              'desc': 'КОМБО: +20 % распр., +25 % скрытность.',
                              'cost_tier': 'combo'},
    ...
    'cytokine': {'name': 'Цитокиновый шторм', 'emoji': '☣', 'x': 88, 'y': 64, 'max': 1,
                 ...
                 'desc': 'СУПЕР: −40 % вакцина, +30 % смертность.',
                 'cost_tier': 'final'},
    'autoimmune': {'name': 'Автоиммунный коллапс', 'emoji': '☠', 'x': 88, 'y': 76, 'max': 1,
                   ...
                   'desc': 'ФИНАЛ: −60 % вакцина, +50 % заметность.',
                   'cost_tier': 'legendary'},
...
def upgrade_cost(node_id, current_level, type_mult):
    ...
    tier = COST_BY_TIER[node['cost_tier']]
...
# --------------------------------------------------------------------------
def _gen_code():
    return 'PL-' + ''.join(random.choice('ABCDEFGHJKLMNPQRSTUVWXYZ23456789') for _ in range(4))
...
def new_disease(dtype=None):
    return {
        'type': dtype,  # None до выбора в лобби
        'infected': START_INFECTED,
        'peak_infected': START_INFECTED,
        'killed': 0,
        'visibility': START_VIS,
        'detected': False,
        'cure': 0.0,
        'neutralized': False,
        'neutralized_at': None,
...
def compute_effects(d):
    eff = {'spread': 0.0, 'kill': 0.0, 'stealth': 0.0, 'visibility': 0.0,
           'vaccine_slow': 0.0, 'om_rate': 0.0}
    if d['type'] is None:
        return eff
    dt = DISEASE_TYPES[d['type']]
    org_mult = dt['organ_mult']
    ...
    for nid, lvl in d['skills'].items():
        ...
        sys_mult = org_mult.get(node.get('system', 'root'), 1.0)
...
# --------------------------------------------------------------------------
# ОБЩИЙ ПУЛ и такт мира
# --------------------------------------------------------------------------
def _rebalance_pool(g):
    """Защита от рассинхрона: если сумма заражённых > живых — пропорционально сжать."""
...
def world_tick(g):
    ...
    # ---------- Рост: ОБЩИЙ ПУЛ ----------
    # Сначала каждый желает, потом пропорционально урезаем.
    claims = []  # (disease, desired_new)
    for d in g['diseases']:
        ...
        # Логистика относительно всего населения
    ...
    if total_desired > free_pool and total_desired > 0:
        scale = free_pool / total_desired
    else:
        scale = 1.0
    ...
    # ---------- Смерти: каждая болезнь убивает своих ----------
    for d in g['diseases']:
...
def update_cure(g):
    for idx, d in enumerate(g['diseases']):
        ...
        if d['cure'] >= CURE_MAX:
            ...
            add_log(g, '💉 Болезнь %s нейтрализована вакциной!' % g['names'][idx])
        ...
        if dtype == 'bacteria':
            ...
            if d['passive_counter'] >= 20:
                ...
                if opened:
                    ...
                    d['skills'][nid] += 1
                    add_log(g, '🦠 %s: %s ур. %d' % (
                        g['names'][idx], SKILL_NODES[nid]['name'], d['skills'][nid]))
            ...
            if d['passive_counter'] >= 15:
                ...
                for nid, lvl in d['skills'].items():
                    if lvl > 0:
                        for req_id, _ in SKILL_NODES[nid]['requires']:
                            critical.add(req_id)
                up = [nid for nid, lvl in d['skills'].items()
                      if 0 < lvl < SKILL_NODES[nid]['max']]
                dn = [nid for nid, lvl in d['skills'].items()
                      if lvl > 0 and nid not in critical]
                if random.random() < 0.5 and up:
                    nid = random.choice(up)
                    d['skills'][nid] += 1
                    add_log(g, '🧬 %s: +1 %s' % (g['names'][idx], SKILL_NODES[nid]['name']))
                elif dn:
                    nid = random.choice(dn)
                    d['skills'][nid] -= 1
                    add_log(g, '🧬 %s: −1 %s' % (g['names'][idx], SKILL_NODES[nid]['name']))
...
def check_over(g):
    ...
    if n == 1:
        ...
        return
    kills = sorted(((i, d['killed']) for i, d in enumerate(g['diseases'])),
                   key=lambda x: -x[1])
    top, second = kills[0][1], kills[1][1] if len(kills) > 1 else 0
    if top > 0 and (top - second) / max(top, 1.0) > 0.005:
        g['winner'] = kills[0][0]
        add_log(g, '🏆 Побеждает %s — убито %s'
                % (g['names'][g['winner']], _fmt(top)))
    else:
        g['winner'] = -1
        add_log(g, '🤝 Ничья')
...
def do_upgrade(g, pid, node_id):
    ...
    if d['skills'].get(node_id, 0) >= node['max']:
        return 'Узел уже максимален'
    for req_id, req_lvl in node['requires']:
        if d['skills'].get(req_id, 0) < req_lvl:
            return 'Требуется %s ур. %d' % (SKILL_NODES[req_id]['name'], req_lvl)
    dt = DISEASE_TYPES[d['type']]
...
def create_game(name, mode=2, disease_type=None):
    ...
    with LOCK:
        ...
        g['tokens'][0] = tok
        g['names'][0] = name
        if disease_type in DISEASE_TYPES:
            g['dtypes'][0] = disease_type
        add_log(g, '%s создаёт партию' % name)
    return code, tok, None


def join_game(code, name, disease_type=None):
    with LOCK:
        g = GAMES.get(code)
        if g is None:
...
def _node_view(node_id, node, d, can_act):
    lvl = d['skills'].get(node_id, 0) if d['type'] else 0
    reqs_info = []
    reqs_ok = True
    ...
    if d['type']:
        ...
        can = (can_act and lvl < node['max'] and reqs_ok
               and d['skill_points'] >= cost)
    else:
        cost = 0
        can = False
    return {
...
def _empty_disease(idx, name, dtype):
    return {
        ...
        'cure': 0.0, 'neutralized': False, 'extinct': False,
        'is_me': True,
        'skills': {nid: 0 for nid in SKILL_NODES},
...
def build_state(g, pid):
    ...
    if g['phase'] == 'lobby' or not g['diseases']:
        ...
        base.update({
            ...
            'all_ready': (all(g['occupied']) and
                          all(dt is not None for dt in g['dtypes'])),
        })
        return base
    ...
    for i, d in enumerate(g['diseases']):
        diseases_view.append({
            ...
            'peak_infected': int(d['peak_infected']),
            'killed': int(d['killed']),
    ...
    nodes_view = [_node_view(nid, n, me_raw, can_act)
    ...
    base.update({
        ...
        'phase': g['phase'],
        'my_name': g['names'][pid],
        'my_dtype': g['dtypes'][pid],
        'world_pop': WORLD_POP,
        'world_alive': total_alive,
